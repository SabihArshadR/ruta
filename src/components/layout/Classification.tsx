    ...
    <div className="min-h-[80vh] bg-white">
      ...
      <div className="w-full">
        <div
         ...
         border-[#E7E7E7] text-backblack font-bold pl-[13px] text-lg"
        >
        ...
        {users?.map((user) => (
          <div
            key={user._id}
            className="grid grid-cols-3 border-b-1 border-lightborder pl-[16px] text-center text-backblack text-lg py-[22px]"
          >
            <div className="text-left font-medium">{user.firstName}</div>
            <div className="truncate font-medium">{user.lastName|| "na"}</div>
            <div className="text-green font-medium text-right pr-[26px]">
              {user.points}
            </div>
          </div>
        ))}
