...
const SplashPopUp = ({ handleClose }: { handleClose: () => void }) => {
  ...
  const playZoomInSound = useCallback(() => {
    try {
      const audio = new Audio('/button-sounds/7.mp3');
  ...
  return (
    <>
      {selectedImage && (
        ...
        >
          <button 
            className="absolute top-4 z-50 right-4 text-white hover:text-gray-300 p-2"
            onClick={(e) => {
              e.stopPropagation();
              closeZoom();
            }}
      ...
      <div className="fixed inset-0 p-2 flex items-center justify-center bg-black/50 z-40">
      <div className="relative w-full max-w-md bg-[#F5F3ED] rounded-2xl shadow-2xl p-3 overflow-y-scroll max-h-[85vh]">
        ...
        <div className="p-4">
          <h2 className="text-[20px] text-backblack font-bold mb-6 text-center">{t("historytitle")}</h2>
