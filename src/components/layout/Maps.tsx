...
const Maps = () => {
  ...
  return (
    <div className="bg-white min-h-[80vh]">
      ...
      <div className="relative bg-[#AD3F37] flex items-center justify-center px-8 py-24 slope-bg top-[40px]">
        <h1
          className={`font-oswald text-4xl leading-[44px] font-bold text-white captalize text-center ${
            destinationTitle === "ninth_title" ? "px-[40px]" : "px-5"
          }`}
        >
          {t(destinationTitle)}
        </h1>
      ...
      <div className="bg-white pb-10">
        ...
        <div className="flex flex-col justify-center items-center px-3.5 mt-[10vh]">
          <CustomButton
            onClick={() => {
              if (poiIdParam) {
                // router.push(`/ar?lat=${latParam}&lng=${lngParam}&poiId=${poiIdParam}`);
                router.push(
                  `/poiIntro?lat=${latParam}&lng=${lngParam}&poiId=${poiIdParam}`
                );
              } else {
                // fallback to previous behaviour: navigate to interactive map to collect coins
                router.push("/mapp");
              }
              playButtonSound();
            }}
