...
const Page = () => {
  ...
  // If this page is opened for a specific POI from the interactive map flow we will receive its id
  const poiIdParam = searchParams.get("poiId");
  const selectedPoiId = poiIdParam ? Number(poiIdParam) : null;
  const router = useRouter();
  const t = useTranslations("Progress");
  const t2 = useTranslations("gameText");
  const selectedBombRef = useRef<string | null>(null);
  ...
  // Try to play audio on initial load (may be blocked by browser)
  useEffect(() => {
  ...
  useEffect(() => {
    if (selectedBombRef.current) return;

    const randomIndex = Math.floor(Math.random() * BOMB_MODELS.length);
    selectedBombRef.current = BOMB_MODELS[randomIndex];

    console.log("🎲 Selected bomb model:", selectedBombRef.current);
  }, []);
  ...
  useEffect(() => {
    ...
    const loadAll = async () => {
      try {
        ...
        if (
          (window as any).AFRAME &&
          !(window as any).AFRAME.components["brighten-material"]
        ) {
  ...
  useEffect(() => {
    ...
    // --- PLAY MUSIC HERE WHEN GAME STARTS ---
    if (backgroundMusicRef.current) {
      backgroundMusicRef.current.volume = 0.05;
      backgroundMusicRef.current.loop = true;
      backgroundMusicRef.current
        ...
        .catch((err) => console.log("Music waiting for interaction"));
    }
  }, [scriptsLoaded, coinConfig]);
    ...
    const timeoutId = setTimeout(() => {
      ...
      floatingRocks.forEach((rock) => {
        ...
        if (
          rock.visible &&
          !rock.disappearing &&
          !eventListenersRef.current.has(rock.id)
        ) {
  ...
  const handleRockTap = (rockId: string) => {
    ...
    setFloatingRocks((prev) => {
      ...
      if (
        ...
        spawnedCoinIndexRef.current < coinConfig.length &&
        updated.length < MAX_VISIBLE_ROCKS
      ) {
    ...
    if (totalCollected >= totalNeeded) {
      ...
      setTimeout(() => {
        // Navigate to the matching quiz for this POI
        const nextPoiForQuiz = selectedPoiId || (user?.POIsCompleted ?? 0) + 1;
        router.push(`/volcano/${nextPoiForQuiz}`);
      }, 100);
    }
  };
      ...
      >
        ...
        {floatingRocks.map((rock) => (
          <AEntity
            key={rock.id}
            id={rock.id}
            gltf-model={`url(${selectedBombRef.current})`}
            brighten-material
